...
public class HydroPump : Skill
{
    ...
    void Update()
    {
        ...
        if (RayTimer >= 3.0f)
        {
            lineRenderer.startWidth -= Time.deltaTime * LeserStartWidth / 1.5f;
            lineRenderer.endWidth -= Time.deltaTime * LeserEndWidth / 1.5f;
            StartVFX.transform.localScale -= Time.deltaTime * new Vector3(1, 1, 1) / 1.5f;
            EndVFX.transform.localScale -= Time.deltaTime * new Vector3(1, 1, 1) / 1.5f;
            if (RayTimer >= 3.8f) { Destroy(gameObject); }
        }
    }
    ...
    {
        MaxLength = Mathf.Clamp(MaxLength + 8.0f * Time.deltaTime, 0, 10);
        ...
        if ((hitinfo) || (hitinfoTop) || (hitinfoBottom))
        {
            ...
            if (EndRay.collider != null && EndRay.collider.gameObject.tag == "Empty")
            {
                ...
                if (e != null)
                {
                    ...
                    {
                        e.SpeedChange();
                        e.SpeedRemove01(1.5f * e.OtherStateResistance);
                    }
                }
            }
            //如果有击中对象，将起始点和终点分别对应
