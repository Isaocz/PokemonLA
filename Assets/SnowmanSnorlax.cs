...
public class SnowmanSnorlax : Empty
{
    public Yamn yamn;
    ...
    /// 冰雾
    /// </summary>
    public GameObject SnowmanSnorlaxFrozenMist;
    ...
    void Update()
    {
        ...
        if (!isBorn)
        {
            ...
            if (!isSleepDone && !isCanNotMoveWhenParalysis)
            {
                ...
                if (EmptyHp <= maxHP * 0.35f && !isGigaImpact && !isSecondGigaImpact)
                {
                    isSecondGigaImpact = true;
                    isGigaImpact = true;
                    animator.SetTrigger("GigaImpact");
                }
                ...
                if (!isGigaImpact && (player.isSleepDone || player.isPlayerFrozenDone) && isAngry && !isEmptyFrozenDone && !isFearDone)
                {
    ...
    /// 发射冰雾
    /// </summary>
    public void LunchFrozenMist()
    {
        Instantiate(SnowmanSnorlaxFrozenMist , transform.position , Quaternion.identity);
    }
