...
public class Solrock : Empty
{
    ...
    public SolrockStoneEdgeManger StoneEdge;
                    ...
                    case State.Normal:
                        ...
                        if (MoveTurnTimer >= 2.0f)
                        {
                            ...
                            if (AttackCount > 0)
                            {
                                ...
                                if (!isFearDone) { Instantiate(StoneEdge, transform.position, Quaternion.identity).ParentEmpty = this; }
                            }
                            MoveTurnTimer = 0.0f;
                            Director = Quaternion.AngleAxis(Random.Range(0, 360), Vector3.forward) * Vector3.right;
