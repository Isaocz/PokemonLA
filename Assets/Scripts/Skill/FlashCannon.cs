...
public class FlashCannon : Skill
{
    ...
    private void rayPosition()
    {
        ...
        RaycastHit2D hitinfoTop = Physics2D.Raycast(transform.position + Vector3.up * 0.3f, transform.right, MaxRange, LayerMask.GetMask("Empty", "EmptyFly", "Enviroment", "Room"));
        RaycastHit2D hitinfoBottom = Physics2D.Raycast(transform.position - Vector3.up * 0.3f, transform.right, MaxRange, LayerMask.GetMask("Empty", "EmptyFly", "Enviroment", "Room"));
        ...
        if ((hitinfo) || (hitinfoTop) || (hitinfoBottom))
        {
            ...
            if (EndRay.collider != null && EndRay.collider.gameObject.tag == "Empty")
            {
                ...
                {
                    isSpDDown = true;
                    if (Random.Range(0.0f,1.0f) + ((float)player.LuckPoint / 30.0f) >= 0.9f) {
